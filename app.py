...
def initialize_agent():
    ...
    # Inicialização da LLM (gemini-pro é estável para tool calling)
...
def detect_outliers_isolation_forest(df: pd.DataFrame) -> dict:
    ...
    """
    try:
        # CORREÇÃO DA SINTAXE: Lista de features deve ser construída corretamente
...
def find_clusters_kmeans(df: pd.DataFrame, n_clusters: int) -> dict:
    ...
    """
    try:
        if n_clusters < 2 or n_clusters > len(df) / 100: 
            return {"status": "error", "message": f"O número de clusters ({n_clusters}) deve ser positivo e razoável para o tamanho do dataset."}
